...
export default function Tasks(props) {
  ...
  const updateTaskInState = (task: Task, updates: Partial<Task>) => {
    const getUpdatedTasks = (taskArray: Task[]) => taskArray?.map(tsk => tsk?.id == task?.id ? ({ ...tsk, ...updates }) : tsk);
  ...
  const changeLabel = (e, task: Task) => {
    ...
    const rawName = cleanedValue;
    const updatedTaskURLs = getItemOrTaskURLs(task, [rawName]);
    const name = stripURLsFromString(rawName, updatedTaskURLs);
